...
const projectDetails = {
  ...
  portfolio: {
    summary: [
      'React 기반 개인 포트폴리오 웹사이트 구축 프로젝트',
      'UI/UX 디자이너와 Figma로 협업한 디자인을 바탕으로 프론트엔드 구현 전반을 단독 담당하고 반응형·접근성·성능·테스트까지 개선',
    ],
    description: [
    ...
    sections: [
      {
        id: 'features', title: '✨ 주요 구현 기능', items: [
          feature('figma-collaboration', 'Figma 기반 디자인 협업 및 UI 구현', [
            'UI/UX 디자이너와 Figma 시안을 공유하며 화면 구성, 색상, 타이포그래피, 인터랙션을 조율',
            '시안을 바탕으로 React와 SCSS를 활용해 페이지와 컴포넌트를 구현하고 프론트엔드 개발 전반을 단독으로 담당',
          ]),
          feature('react-implementation', 'React 기반 프론트엔드 구현 전반 담당', [
            '포트폴리오의 화면 구성, 컴포넌트 설계, 상태 처리, 라우팅, 스타일 구현을 단독으로 담당하여 React 기반 SPA로 구축',
            '프로젝트 카드·상세 화면·경력·기술 스택을 공통 UI와 데이터 구조로 구성해 콘텐츠를 일관된 방식으로 보여주도록 구현',
          ]),
          feature('project-records', '프로젝트 기록 데이터 구조화', [
            '프로젝트 카드, 상세 설명, 기술 스택, 작업 화면 정보를 데이터 중심으로 구성하여 새 프로젝트를 동일한 양식으로 추가할 수 있도록 설계',
            '공통 상세 화면 컴포넌트를 사용해 프로젝트별 마크업 중복을 줄이고 콘텐츠 수정 범위를 데이터 파일로 제한',
          ]),
          feature('responsive-ui', '반응형 포트폴리오 UI 구현', [
            '데스크톱과 모바일 환경에서 프로젝트 및 경력 정보를 읽기 쉽게 확인할 수 있도록 반응형 레이아웃과 모바일 슬라이드 UI를 적용',
            '프로젝트 카드, 아코디언, 이미지 갤러리를 키보드로도 이용할 수 있도록 접근 가능한 인터랙션 구조를 적용',
          ]),
          feature('codex-refactoring', 'Codex 기반 코드 리뷰 및 리팩터링', [
            '초기 구현 이후 AI Coding Tool `Codex`를 코드 리뷰·리팩터링 보조 도구로 활용해 중복 컴포넌트, 상태·이벤트 처리, 접근성 및 오류 대응 지점을 점검',
            '공통 컴포넌트와 커스텀 Hook으로 구조를 정리하고 라우팅·프로젝트 상세 인터랙션·클립보드 동작을 자동 테스트로 검증',
          ]),
          feature('performance-testing', '성능 최적화 및 자동 테스트', [
            '용량이 큰 Spline 3D 요소와 프로젝트 상세 화면을 필요 시점에 불러오고 이미지에 지연 로딩을 적용해 초기 로딩 부담을 줄임',
            '라우팅, 상세 화면 인터랙션, 프로젝트 카드, 클립보드 동작을 자동 테스트로 검증하고 프로덕션 빌드로 배포 전 동작을 확인',
          ]),
        ],
      },
      {
        id: 'troubleshooting', title: '💫 Trouble Shooting', items: [
          feature('detail-duplication', '프로젝트 상세 화면의 중복 구조 관리', [
            '**[문제점]** 프로젝트마다 별도 상세 화면을 유지하던 구조는 동일한 마크업과 인터랙션 코드가 반복되어 새 프로젝트 추가 및 공통 기능 수정 시 변경 범위가 넓어질 수 있었음',
            '**[해결]** 프로젝트 설명, 기술 스택, 기능 목록, 작업 화면을 데이터 파일로 분리하고 공통 `ProjectDetail` 컴포넌트로 렌더링하도록 리팩터링',
            '**[회고]** 콘텐츠와 UI 로직의 책임을 분리하면 작업 기록이 늘어나는 개인 사이트에서도 확장성과 유지보수성을 높일 수 있음을 경험했습니다.',
          ]),
          feature('lazy-loading', '초기 로딩 리소스 관리', [
            '**[문제점]** 첫 화면의 3D 요소와 프로젝트별 이미지·상세 모듈을 동시에 로드하면 초기 진입 시 불필요한 리소스가 포함될 수 있었음',
            '**[해결]** `React.lazy`와 `Suspense`로 Spline과 프로젝트 상세 화면을 필요 시점에 로드하고 이미지에 `loading="lazy"`를 적용',
            '**[회고]** 초기 화면에 반드시 필요한 리소스와 이후에 필요한 리소스를 구분해 로딩 전략을 설계하는 것이 체감 성능 개선에 중요함을 경험했습니다.',
          ]),
          feature('interaction-accessibility', '카드·모달 인터랙션 접근성 보완', [
            '**[문제점]** 프로젝트 카드의 중첩 클릭 구조와 이미지 모달의 키보드 제어 방식은 중복 이벤트 또는 키보드 이용의 불편으로 이어질 가능성이 있었음',
            '**[해결]** 카드 전체를 하나의 버튼으로 정리하고 이미지 모달에 포커스 이동·`Escape` 닫기·방향키 탐색을 적용해 상호작용을 보완',
            '**[회고]** 시각적 구현뿐 아니라 입력 방식별 사용자 흐름과 이벤트 전파를 함께 검토해야 안정적인 UI를 만들 수 있음을 경험했습니다.',
          ]),
        ],
      },
    ],
